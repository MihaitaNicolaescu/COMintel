...
public class FacturaDAO {
	...
	public int numberOfFacturi() {
		...
		try {
			conn = ConnectionManager.createConnection();
			statement = conn.createStatement();
			String query ="SELECT COUNT(*) Total FROM FACTURI";
	...
	public ArrayList<Factura> getFacturi(int id_abonat, int page){
		...
		try {
			conn = ConnectionManager.createConnection();
			statement = conn.createStatement();
			result = statement.executeQuery("SELECT * FROM (SELECT a.*, ROWNUM rnum from (SELECT * FROM FACTURI) a WHERE ROWNUM <= " + Stop + ") WHERE rnum >= " + Start + " AND ID_ABONAT = '"+ id_abonat+"'");
			while(result.next()) {
